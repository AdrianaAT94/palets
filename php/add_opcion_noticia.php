...
<?php
            ...
            if (isset($_REQUEST['publicar_noticia'])) {
                $titulo_es = $_REQUEST['titulo_es_noticia'];
                $titulo_es = mysqli_real_escape_string($con, $titulo_es);
                $titulo_gl = $_REQUEST['titulo_gl_noticia'];
                $titulo_gl = mysqli_real_escape_string($con, $titulo_gl);
                $titulo_en = $_REQUEST['titulo_en_noticia'];
                $titulo_en = mysqli_real_escape_string($con, $titulo_en);
                $descripcion_es = $_REQUEST['descripcion_es_noticia'];
                $descripcion_es = mysqli_real_escape_string($con, $descripcion_es);
                $descripcion_gl = $_REQUEST['descripcion_gl_noticia'];
                $descripcion_gl = mysqli_real_escape_string($con, $descripcion_gl);
                $descripcion_en = $_REQUEST['descripcion_en_noticia'];
                ...
                //IMAGEN
                if (!esImagen($_FILES['imagen_noticia']['tmp_name']) && $_FILES['imagen_noticia']['tmp_name'] != "") {
                ...
                if (is_uploaded_file ($_FILES['imagen_noticia']['tmp_name'])) {
                    ...
                    $nombreFichero = $_FILES['imagen_noticia']['name'];
                    $copiarFichero = true;
                 ...
                 else if ($_FILES['imagen_noticia']['error'] == UPLOAD_ERR_FORM_SIZE) {
                    $maxsize = $_REQUEST['MAX_FILE_SIZE'];
                    $errores = $errores . "   <p>El tamaño de la imagen supera el límite permitido ($maxsize bytes)</p>";
                }

                else if ($_FILES['imagen_noticia']['name'] == "")
                    $nombreFichero = '';
                else
                ...
                if ($copiarFichero) {
                    move_uploaded_file($_FILES['imagen_noticia']['tmp_name'], "../". $nombreDirectorio . $nombreFichero);
                }
                ...
                mysqli_query($con, "insert into noticias (codigo, nombre_es, nombre_gl, nombre_en, fecha, autor, imagen, texto_es, texto_gl, texto_en) values ('$key','$titulo_es','$titulo_gl','$titulo_en','$fecha','$id_administrador[0]','$nombreCompleto','$descripcion_es','$descripcion_gl','$descripcion_en')");
