...
$consulta_ofertas = "SELECT * FROM ofertas";
$rs_ofertas = mysqli_query($con, $consulta_ofertas);

//Contamos numero de registros
$num_total_ofertas = mysqli_num_rows($rs_ofertas);
...
$total_paginas = ceil($num_total_ofertas / $tam_pag);
/*consulta paginada*/
$consulta = "SELECT * FROM ofertas ORDER BY id DESC LIMIT ".$inicio."," . $tam_pag;
...
$registros = '';  
if (isset($_POST['buscar_oferta'])) {

    $busqueda = trim($_POST['buscar_oferta']);
    ...
    if (empty($busqueda)){ 
      $texto = '<p class="texto_busqueda">Búsqueda sin resultados<p>'; 
        ...
        $sql = "SELECT * FROM ofertas WHERE nombre_es LIKE '%" .$busqueda. "%' ORDER BY nombre_es";  
        $resultado = mysqli_query($con, $sql); //Ejecución de la consulta 
         //Si hay resultados... 
        if (mysqli_num_rows($resultado) > 0){  
            echo '<table class="tabla_ofertas" align="center">
                <tr>
                <th>Listado de Ofertas</th>
                <th>Autor</th>
                <th>Fecha</th>
                </tr>';
            ...
            <div class="iconos_bd"><div class="img1_bd"><a href="?editar_oferta=<?php echo $elem; ?>"><img src ="images/iconos_areaprivada/ap_editar.png"></a></div>
            <div class="img2_bd"><a onclick="if(confirm('ADVERTENCIA \n Si acepta eliminar el archivo, también será eliminado de su servidor. ¿Desea borrar de forma permanente este archivo?') == false){return false;}" href="?borrar_oferta=<?php echo $elem; ?>"><img src="images/iconos_areaprivada/ap_eliminar.png"></a></div><div class="clear"></div></div></div></td>
...
?>
<div class="buscador">
  <form id="buscador_oferta"  method="post" action="">  
    <div class="formu_input"><input id="buscar_oferta" name="buscar_oferta" type="search" placeholder="Buscar..." autofocus > </div>
    <div class="formu_img"><input type="image" src="images/iconos_areaprivada/ap_buscar.png" alt="submit" class="boton" value="buscar"> </div>
    <div class="clear"></div>
  </form>
</div>

<table class="tabla_ofertas" align="center">
<tr>
<th>Listado de Ofertas</th>
<th>Autor</th>
<th>Fecha</th>
...
<?php
echo '<div class="paginacion_ofertas">';
if ($total_paginas > 1) {
    if ($pagina != 1) {
            echo '<a href="?list_ofertas&pagina='.($pagina-1).'">« Anterior</a> | ';
            for ($i=1;$i<=$total_paginas;$i++) {
                if ($pagina == $i)
                    ...
                    echo '  <a class="color" href="?list_ofertas&pagina='.$i.'">'.$i.'</a>  ';
           }
           if ($pagina != $total_paginas)
               echo '<a href="?list_ofertas&pagina='.($pagina+1).'"> | Siguiente »</a>';
} } }
echo "</div>";
mysqli_close($con); 
?>